...
// Class:      CustomVertexTableProducer
//
/**\class CustomVertexTableProducer CustomVertexTableProducer.cc PhysicsTools/CustomVertexTableProducer/plugins/CustomVertexTableProducer.cc
...
class CustomVertexTableProducer : public edm::stream::EDProducer<> {
public:
  explicit CustomVertexTableProducer(const edm::ParameterSet&);
  ~CustomVertexTableProducer() override;
...
//
CustomVertexTableProducer::CustomVertexTableProducer(const edm::ParameterSet& params):
...
CustomVertexTableProducer::~CustomVertexTableProducer()
{
...
void
CustomVertexTableProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  for (const auto & sv : *svsIn) {
    if (svCut_(sv)) {
      ...
      if(dl.value() > dlenMin_ and dl.significance() > dlenSigMin_){
        selCandSv->push_back(svsIn->ptrAt(i));
        dlen.push_back(dl.value());
        dlenSig.push_back(dl.significance());
        reco::Candidate::Vector p = sv.momentum();
        reco::Candidate::Vector d(sv.vx() - PV0.x(), sv.vy() - PV0.y(), sv.vz() - PV0.z());
        double cosAngle = p.Unit().Dot(d.Unit());
        pAngle.push_back(std::acos(cosAngle));
...
void
CustomVertexTableProducer::beginStream(edm::StreamID)
{
}

// ------------ method called once each stream after processing all runs, lumis and events  ------------
void
CustomVertexTableProducer::endStream() {
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
CustomVertexTableProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(CustomVertexTableProducer);
